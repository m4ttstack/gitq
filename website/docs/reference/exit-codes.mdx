...
| `reparent` | `src/cli/commands/surgery.ts:226`: `cascadeResults.every((r) => r.success) ? 0 : 1`, over the descendant cascade's own results | `result.cascadeResult.results[].success`, the same shape |
| `absorb` | `src/cli/commands/surgery.ts:110-121`: exits `1` when `result.attributions.some((a) => !a.success)`, and also when `result.recovery` is set, which means work of yours is sitting somewhere you have to go get | `result.attributions[].success`, each a `{ branch, files, success, error? }` |
