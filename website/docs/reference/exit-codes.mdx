...
- A `sync` whose remote trunk ref does not resolve, even though the fetch before it succeeded: `gitq: cannot sync: origin/main does not resolve after fetching origin (remote "origin" has no branch "main"; push the stack root first: git push -u origin main). nothing was rebased`. Thrown from `syncLocalStack` (`src/core/rebase-engine.ts:1244-1252`), so it reaches you through `main.ts`'s catch rather than a `fail()` call. This case used to exit `0` with an empty result; a `0` from `sync` now means the stack was actually rebased onto `origin/<root>`. See [`gitq sync`](/reference/cascade/sync#behavior).
- A missing GitLab token, for `publish`/`import` (`src/cli/provider.ts`).
...
| `reparent` | `src/cli/commands/surgery.ts:207`: `cascadeResults.every((r) => r.success) ? 0 : 1`, over the descendant cascade's own results | `result.cascadeResult.results[].success`, the same shape |
| `absorb` | `src/cli/commands/surgery.ts:94-102`: exits `1` when `result.attributions.some((a) => !a.success)` | `result.attributions[].success`, each a `{ branch, files, success, error? }` |
| `publish` | `src/cli/commands/forge.ts:109-114`: `result.results.every((r) => r.success)` | `results[].success`, each a `{ branch, success, action, mrIid?, mrUrl?, error?, changes?, targetBranch? }` |
