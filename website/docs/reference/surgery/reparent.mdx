...
Guarded by the stack lease, and always runs detached in a leased [work slot](/concepts/work-slots) (`src/cli/commands/surgery.ts:209-231`). If `--onto` names the branch's current parent, it is a no-op: returns immediately, nothing rebased (`src/core/reparent.ts:47-55`).
