...
Once nodes exist, each is a plain `StackNode`: `branch`, `parent`, `mrIid`, `mrUrl`, `mrTitle`, `status` (`local-only | synced | drift | merged`), `lastKnownHead`, `forkPoint`, `diffStats`, `pipelineStatus`, `unresolvedThreads`. `stacks` does not add `checkedOutIn`, or any other worktree information, to a node; that enrichment belongs to `diagnose`, below. To learn where a branch from `stacks` is checked out, cross reference the top level `worktrees` array's own `branch` field yourself.
...
`src/cli/commands/surgery.ts:40-105`. `--preview` emits `{ stack, result }` where `result` is an `AbsorbPreview` (`src/core/absorb.ts:27-31`: `attributed` a `Record<branch, filePath[]>`, `unattributed` a `string[]`, `currentBranch`). Real capture, a clean tree with nothing to attribute:
...
A committing (non `--preview`) `absorb` emits `{ stack: updatedStack, result }`, where `result` is an `AbsorbResult` (`src/core/absorb.ts:19-24`, described from source, not captured live): `absorbed` (boolean), `reason` (`'no-changes'`, optional), `attributions` (`AbsorbAttribution[]`: `{ branch, files, success, error? }`), `cascadeResult` (optional `CascadeResult`, the same shape `sync` produces, for the restack absorb runs afterward), `updatedStack` (optional).
