...
`src/cli/commands/surgery.ts:41-124`. `--preview` emits `{ stack, result }` where `result` is an `AbsorbPreview` (`src/core/absorb.ts:36-43`: `attributed` a `Record<branch, filePath[]>`, `unattributed` a `string[]`, `currentBranch`). `unattributed` is what absorb will leave dirty in the worktree, not a branch assignment; see [`gitq absorb`](/reference/surgery/absorb). Real capture, a clean tree with nothing to attribute:
