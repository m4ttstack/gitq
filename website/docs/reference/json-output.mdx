...
`publish` (`src/cli/commands/forge.ts:60-93`) emits `{ results, updatedStack }`, where `results` is `PublishNodeResult[]` (`src/core/forge-sync.ts:37-43`): `{ branch, success, mrIid?, mrUrl?, error? }`.
