...
3. Only then is the forge provider built (`createForgeProvider`, `src/cli/provider.ts:44-78`), which is where the remote's host decides both the forge and the credential, and where the missing-token refusal happens: `no gitlab token for gitlab.com (set GITLAB_TOKEN or add gitlabToken to ~/.mattstack/rt/secrets.json)`. A remote whose host names no forge gitq knows is refused here too, rather than guessed at. See [Configuration](/reference/configuration#the-forge-token).
