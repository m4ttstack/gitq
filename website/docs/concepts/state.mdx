...
The GitLab token for `publish` and `import`: `GITLAB_TOKEN` from the environment first, then the `gitlabToken` field in `~/.mattstack/rt/secrets.json`. gitq never writes either. See [Configuration](/reference/configuration).
