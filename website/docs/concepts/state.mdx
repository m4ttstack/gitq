...
| `<app root>/settings.json` | `maxWorkSlots`, `workSlotLocation`, `forges` -- the fallback for `gitq.workSlots`/`gitq.forges` in the [settings store](/reference/configuration#the-settings-store), until each is imported | Yes. Falls back to the defaults. |
...
| `<app root>/config.json` | Board configuration -- the fallback for `gitq.board` in the [settings store](/reference/configuration#the-settings-store), until it's imported | It is yours to manage. |
...
The [work slots](/concepts/work-slots) gitq leases for cascades follow the config directory with no carve-out: a non-pool repo's slots are created under `<config dir>/work/<hash>/` (`getWorkSlotRoot`, `src/core/config-paths.ts`).
...
Both of these live in the [app root](#the-app-root-and-the-config-directory-inside-it) and, unlike the CLI's files, do not follow `GITQ_CONFIG_DIR`: a throwaway stack store should not also repoint the board.
