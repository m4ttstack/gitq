...
- An entry is written only when the command succeeds. A failed command logs nothing, and a cascade that pauses (exit `2`) logs nothing either, because a pause is resolved with `continue` or `abort`, not with `undo`.
