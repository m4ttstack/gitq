...
The [work slots](/concepts/work-slots) gitq leases for cascades follow the same variable, with one deliberate exception for the default. Set `GITQ_CONFIG_DIR` and a non-pool repo's slots are created under `$GITQ_CONFIG_DIR/work/<hash>/` (`getWorkSlotRoot`, `src/core/config-paths.ts:52-54`); leave it unset and the root stays at `~/.cache/gitq/work/<hash>/`, outside the config directory, so slots you already have leased are still found. That is what makes a throwaway `GITQ_CONFIG_DIR` genuinely throwaway: it sandboxes the git worktrees gitq creates, not just the files listed here.
