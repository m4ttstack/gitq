...
| `~/.config/gitq/settings.json` | `maxWorkSlots`, `workSlotLocation`, `forges` | Yes. Falls back to the defaults. |
...
The keys gitq reads from it are `maxWorkSlots`, `workSlotLocation`, and `forges`:

```json
{ "maxWorkSlots": 5, "workSlotLocation": "root" }
```

`maxWorkSlots` must be a number of at least 1. Anything else, or no file at all, gives the default of 3. `workSlotLocation` is `auto` (the default: slots join a pool's worktrees) or `root` (slots always go under the work-slot root). gitq never creates or writes this file; make it yourself if you want to change a value. See [Work slots and leases](/concepts/work-slots) and [Configuration](/reference/configuration).
