...
| `completedBranches` | Branches *this invocation's* walk has already rebased successfully. It is not the whole cascade's history: every `gitq continue` starts a fresh walk over the remaining branches, so this list resets to empty on resume even if earlier branches finished in a prior invocation. Read `preRebaseHeads` for the whole run's history. |
...
- It does **not** rewind the branches the cascade already finished. Those refs were moved and stay moved, whether or not `pauseInfo.completedBranches` still shows them: that field only covers the current invocation's walk and resets to empty on every `gitq continue`, so `preRebaseHeads` is the field with the whole run's history.
