...
- It does **not** rewind the branches the cascade already finished. Those refs were moved and stay moved, whether or not `pauseInfo.completedBranches` still shows them: that field only covers the current invocation's walk. It is usually empty again after a `gitq continue` that resumes into a fresh walk, but a `continue` that itself re-conflicts carries the prior pause's list forward unchanged, so `preRebaseHeads` is the field with the whole run's history.
