...
- If `origin/<root>` does not resolve after that fetch, sync has no target to rebase onto and refuses: exit `1`, nothing rebased, `gitq: cannot sync: origin/<root> does not resolve after fetching origin (...)`, where the parenthetical names what to go check and how to fix it. One of its two arms does blame a missing `origin`, which reads oddly next to the bullet above, and it is exotic by construction: it only fires when the fetch *succeeded* with no remote of that name, as when git resolves `origin` as a path or `origin` names a fetch group. The everyday remote-less repo never gets that far. Sync does not fall back to your local root either: it never fetches into that branch, so rebasing onto it would report success while leaving the stack based on stale history. See [`gitq sync`](/reference/cascade/sync#behavior).
