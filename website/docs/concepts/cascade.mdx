...
- `gitq sync` runs `git fetch origin` first, every time, before anything else. If the fetch fails, sync throws and exits `1` with `gitq: Failed to fetch from remote: ...`. **A repo with no `origin` remote cannot sync**, and that is the failure it hits: the fetch dies before any ref is looked at.
- If `origin/<root>` does not resolve after that fetch, sync has no target to rebase onto and refuses: exit `1`, nothing rebased, `gitq: cannot sync: origin/<root> does not resolve after fetching origin (...)`, where the parenthetical names what to go check and how to fix it. One of its two arms blames a missing `origin`, which sounds like the bullet above but is a different, exotic case: it only fires when the fetch *succeeded* with no remote of that name, as when git resolves `origin` as a path or `origin` names a fetch group. The everyday remote-less repo dies at the fetch and never gets this far. Sync does not fall back to your local root either: it never fetches into that branch, so rebasing onto it would report success while leaving the stack based on stale history. See [`gitq sync`](/reference/cascade/sync#behavior).
...
gitq sync
  ...
  ├─ resolve origin/<root>                 throws when unresolvable
...
- **It does not push.** After a sync, every rebased branch has diverged from its published counterpart and needs a force push. The JSON output's `rebasedBranches` lists exactly which ones. See [Publish a stack](/guides/publish).
