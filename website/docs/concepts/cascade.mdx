...
A cascade is gitq rebasing a whole tree of branches in one pass, parents before children, each branch onto its parent's new head. `gitq sync` is the cascade you will run most; `reparent`, `fold`, `split --files`, and `absorb` each finish by cascading whatever they disturbed. This page is what all of them do.
...
`gitq preflight` warns about the branches that will need reconciling before you run any of this. See [Reading the tree](/getting-started/reading-the-tree).
...
- A cascade that dies halfway, crashes, or is killed leaves your working tree untouched. The abandoned work in the slot is just a detached HEAD, and it is discarded on the next acquisition.
...
`gitq sync` fetches and rebases the entire stack onto the remote trunk. The surgery commands run the same loop with a narrower scope and no fetch: they cascade only the descendants of the branch they changed, onto that branch's current local head. Same pause protocol, same work slot, same compare and swap, same output shape.
