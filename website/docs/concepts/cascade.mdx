...
- If `origin/<root>` does not resolve after that fetch, sync has no target to rebase onto and refuses: exit `1`, nothing rebased, `gitq: cannot sync: origin/<root> does not resolve after fetching origin (...)`, where the parenthetical names the cause, a missing `origin` remote or a stack root that was never pushed. It does not fall back to your local root: sync never fetches into that branch, so rebasing onto it would report success while leaving the stack based on stale history. See [`gitq sync`](/reference/cascade/sync#behavior).
