...
The work-slot root follows gitq's config directory: set `GITQ_CONFIG_DIR` and slots are created under `$GITQ_CONFIG_DIR/work/<hash>/` instead (`getWorkSlotRoot`, `src/core/config-paths.ts:52-54`). One variable moves the whole footprint, so a throwaway `GITQ_CONFIG_DIR` really is throwaway: it sandboxes the git worktrees gitq creates, not just the stack store. Leave the variable unset and the root stays at `~/.cache/gitq/work`, out of the config directory, exactly where it has always been, so slots you already have leased are still found. See [Configuration](/reference/configuration).
...
A few commands refuse up front instead of at the end, because they rewrite the branch directly in the worktree you ran them from rather than in a slot: `absorb` (for every branch its preview attributes changes to), `rename`, and `reset` all refuse immediately if the branch is checked out in a worktree other than the current one.
