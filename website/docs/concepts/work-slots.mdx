...
| Anything else, including a plain single-worktree clone | `~/.cache/gitq/work/<hash>/gitq-1`, where `<hash>` is the first 16 hex characters of the SHA-256 of the repo's git common dir. |
...
A slot under `~/.cache/gitq/work/` is not inside your repo, so deleting the repo directory does not delete the slot. Recreate a repo at the same path later and the stale slot is still there, and the next cascade fails with a raw `fatal: '.../gitq-1' already exists` from git. Remove the cache directory too.
