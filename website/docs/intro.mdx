...
- It does surgery on the tree. `absorb` distributes uncommitted changes to the branches that own those files, `split` breaks one branch into two, `fold` collapses a branch into its parent, and `reparent` moves a branch (and its descendants) somewhere else in the tree. Only `reparent` restacks descendants through the same cascade `sync` uses; `absorb` restacks through a separate loop of its own, and `fold` and `split --files` leave descendants for a later `sync`.
...
- **It talks to one forge per repo, resolved from the remote.** `publish` and `import` read the git remote's host to decide whether they are speaking to GitLab or GitHub, and against which instance. gitlab.com and github.com need no configuration; self-hosted GitLab and GitHub Enterprise need a line naming the forge, since a hostname does not say which one it runs. The board is still the exception: it resolves a single provider for every repo it shows, so its MR enrichment is GitLab-only for now.
