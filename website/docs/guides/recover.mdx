...
- **It requires a clean launch worktree**, even though it never checks anything out there:
  ...
  Exit `1`, and nothing moves. Like `rename`, it also refuses up front, with the same message shape as [`rename`'s refusal](/guides/restructure-a-stack#rename), if the branch is checked out somewhere other than your launch worktree, dirty or not. When it does run, it compare-and-swaps the branch ref and leaves your checkout exactly where it was: `gitq reset feat-a` from `main` leaves you on `main`.
