...
Deleted `~/.config/gitq/stacks/<hash>.json` by accident, or the tree in it just doesn't match reality anymore and hand-editing isn't worth it. If the forge still has the MRs, `gitq import --replace` rebuilds the store from there: the reverse direction of `publish`, forge state overwriting local state:
...
Treat it as a recovery tool for a broken store, not a routine refresh: `gitq sync` is what you want for a stack that's merely behind. See [Publish a stack](/guides/publish#import-replace) for the full command reference.
