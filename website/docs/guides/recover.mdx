...
- **It requires a clean launch worktree**, and checks out the branch there before hard-resetting it:
  ...
  Exit `1`, and nothing moves. Like `rename`, it also refuses up front — same message shape as [`rename`'s refusal](/guides/restructure-a-stack#rename) — if the branch is checked out somewhere other than your launch worktree, dirty or not.
...
Every stack `import` creates gets a brand-new random id (`crypto.randomUUID()` in `StackManager.createStack`), with no relationship to whatever id the same stack had before, even if you're importing back the exact MRs you just exported. The name you see in `gitq stacks` is a readable slug derived from the tip branch's MR title, so it can look stable across two imports — the underlying `id` never is. Anything keyed to the old id, most importantly any `gitq undo` history for operations run before the import, stops resolving to a current stack the moment you replace it.
