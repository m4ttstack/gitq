...
2. **Builds the slash command**: `/gitq:<action> <repoPath> <stack> --state <statePath> --status-bin <path/to/gitq>`, and launches a herdr tab running `claude` with that as the initial prompt.
