...
Which token gitq wants follows from the repo's remote, not from configuration: a GitHub remote asks for `GITHUB_TOKEN` (or `githubToken`) and is never offered a GitLab one. The instance comes from the remote too, so self-hosted GitLab works, given a `forges` entry naming it. The board is the exception: it builds one provider for every repo it shows and still gates that on a GitLab token, so its `gitlabHost` config field remains accepted and unwired. See [the forge token](/reference/configuration#the-forge-token).
