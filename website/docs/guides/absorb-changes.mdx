...
1. **Attribute.** For each dirty file, blame the lines the edit actually changes and find the deepest stack branch that owns any of them. That branch owns the file. When blame has no answer (a new file, a binary one, or lines written outside the stack) fall back to the older rule: the deepest branch whose own commits touched that file at all. A file no branch owns either way has no owner, and absorb will not invent one. `--at <branch>` overrides all of it when you know where the fix belongs, and `--at <branch>:<glob>` overrides only the files matching the glob; see [the reference](/reference/surgery/absorb#at).
