...
The attempt can fail — the same failing hook usually refuses the checkout too — and then absorb says so instead of exiting quietly on the wrong branch:

```
nothing absorbed
absorb could not clean up after the failed amend: could not check feat-handlers back out (...) — you are on feat-api; left the stash alone rather than popping your dirty tree onto the wrong branch. Your uncommitted work is retained in stash@{0}: check it with `git stash show -p stash@{0}`, then `git checkout feat-handlers` and `git stash pop`.
```

Your work is in `stash@{0}`, and absorb deliberately did not pop it onto a branch you never picked. Do what the message says: get back to your branch first, then pop.
