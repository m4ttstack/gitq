...
Step 4 waits for step 3 to finish rather than running with it: a dirty launch worktree makes the restack's ref finalization refuse. So there is a window, inside the command, where your unattributed changes exist only in absorb's memory. It restores them even when the restack blows up.
