...
Still on `feat-handlers`, still clean, no rebase in progress, and `feat-api` still at its pre-sync head. gitq rebases in a leased [work slot](/concepts/work-slots) with a detached HEAD and only moves your branch refs at the very end, so a paused cascade leaves the still-conflicted and not-yet-attempted branches alone. It is a different story for a branch the walk already finished: if your clean checkout had been sitting on `feat-api` instead, gitq would have reset it to `feat-api`'s new head the moment that branch's rebase landed, the same way [surgery resets a clean checkout](/guides/restructure-a-stack#what-surgery-does-to-your-checkout) sitting on a branch it rewrites.
