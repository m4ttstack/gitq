...
The out-of-tree root follows gitq's config directory: set `GITQ_CONFIG_DIR` and slots are created under `$GITQ_CONFIG_DIR/work/<hash>/` instead (`getWorkSlotRoot`, `src/core/config-paths.ts:52-54`). Leave the variable unset and the root stays at `~/.cache/gitq/work`, exactly where it has always been. Pool slots are unaffected either way: they are siblings of your own worktrees, wherever those live.
