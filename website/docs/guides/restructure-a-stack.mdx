...
`sync` rebases a stack. It never changes its shape. Changing the shape is surgery: five commands that split a branch in two, merge one into another, move a subtree, rename a node, or snap a branch back to its remote.
