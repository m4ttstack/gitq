...
**Surgery does not switch the branch your checkout is on, with two exceptions: `fold` and `reset` (both below).** `split --files` and `reparent` replay commits detached in a leased [work slot](/concepts/work-slots) and then move the branch ref with a compare and swap; `split --at` and `rename` are pure ref operations that replay nothing at all. Whatever branch you were on, you are still on.
...
:::note The two exceptions are `fold` and `reset`
...
`reset` checks the branch out in your launch worktree before hard-resetting it, whether or not that is the branch you started on. Details in [`reset`](#reset) below.
...
:::danger `--at` needs the full 40 character sha

A short sha is compared against full hashes and never matches:
...
```
gitq: Commit "d66c1c7" not found in branch "feat-core"
```

Exit `1`. Use `git rev-parse <ref>`, not `git rev-parse --short`.
...
| Sha is a full 40 character commit id, but older than the branch's 50 most recent commits | `gitq: Commit "<sha>" not found in branch "<branch>"`, the same shape of message the short sha error above produces, but for a different reason: `--at` only looks at the branch's last 50 commits, so anything older is invisible to it no matter how correctly you typed the sha |
...
```
paused on feat-q in /Users/you/.cache/gitq/work/80c99b99f76bdc37/gitq-1 (commit ?/?):
  UU shared.ts
...
```json
{
  ...
  "pauseInfo": {
    ...
    "worktreePath": "/Users/you/.cache/gitq/work/80c99b99f76bdc37/gitq-1"
  }
}
```
...
Renaming a published branch does not rename it on the remote. The old branch is still on GitLab, and the MR is still attached to it. See [Publish to GitLab](/guides/publish-to-gitlab).

## `reset`: snap a branch back to its remote {#reset}

**When you want it.** Local and remote have diverged, for example someone force-pushed the branch on GitLab, and you want your local ref to match exactly what is there.

`gitq reset <branch>` requires a clean launch worktree: it checks out the branch there and hard-resets it to `origin/<branch>`, so uncommitted changes in the way stop it with `gitq: Working tree has uncommitted changes. Commit or stash first.` before anything moves. It does not fetch on its own, so make sure your remote-tracking ref for the branch is current first.

### Reset is the other exception to checkout neutrality

Checking the branch out is not conditional on it being the branch you already had checked out; `reset` does it regardless. If you were on `feat-a` and run `gitq reset feat-b`, you are left standing on `feat-b`, not back on `feat-a`. Nothing restores your original branch afterward.

Like `rename`, it refuses up front if the branch is checked out somewhere other than your launch worktree, dirty or not, with the same message.
