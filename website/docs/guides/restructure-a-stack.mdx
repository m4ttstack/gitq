...
:::tip `--at` takes any revision git resolves

A short sha copied straight out of `git log --oneline` is fine, as are a full sha, an annotated tag, `HEAD~2`, and a branch name:
...
| Abbreviation matches more than one object | `gitq: Commit "63b2" is an ambiguous abbreviation (matches 63b2796818, 63b2bc6d43); use more characters` |
| Revision resolves to nothing | `gitq: Commit "deadbee" does not resolve to a commit in this repository` |
