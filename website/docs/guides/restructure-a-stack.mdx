...
`sync` rebases a stack. It never changes its shape. Changing the shape is surgery: five commands that split a branch in two, merge one into another, move a subtree, or rename a node.

All of them update gitq's tree and the git refs together, none of them replays a commit inside the worktree you launched them from, and all of them refuse rather than half-finish. Every block below came from a real run.
...
Because it is pure ref surgery, **a dirty working tree is not in the way**. The run above was made with an untracked file sitting in the launch worktree, and it was still there afterwards.
...
| Sha is not among the branch's 50 most recent commits | `gitq: Commit "d66c1c7" not found in branch "feat-core"` |
