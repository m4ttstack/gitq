...
gitq compares the stack root against `origin/<root>`, not your local branch, and branches whose parent is the root rebase directly onto `origin/<root>` when you sync. Your local trunk branch is never rewritten. In a repo with no `origin` remote, `gitq diagnose` cannot see that trunk moved and `gitq sync` fails outright at the fetch. So this tutorial gives the scratch repo a remote: a local bare repository standing in for a forge. No network involved.
...
The scratch repo is disposable: `rm -rf /tmp/gitq-demo /tmp/gitq-demo-origin.git`. That deletes the branches but not gitq's record of them, since the stack store lives in `~/.mattstack/gitq/`. It also does not remove the work slot that `sync` leased for the cascade, which lives outside the repo at `~/.mattstack/gitq/work/<hash>/gitq-1` (the `<hash>` is derived from the repo's git common dir, not from the directory you ran gitq in). If you delete `/tmp/gitq-demo` and recreate it at the same path later, remove that directory too, otherwise the next `gitq sync` hits a raw `fatal: '.../gitq-1' already exists` from git. Drop the record too:
