...
`preflight` replays the rebase in memory to predict conflicts, and tells you whether the working tree is dirty, before anything moves. See [Reading the tree](/getting-started/reading-the-tree).
...
```mermaid
graph TD
  base["main (e29b0ff)"] --> trunk["main (d1cf2c1) add changelog"]
...
The scratch repo is disposable: `rm -rf /tmp/gitq-demo /tmp/gitq-demo-origin.git`. That deletes the branches but not gitq's record of them, since the stack store lives in `~/.config/gitq/`. Drop the record too:
