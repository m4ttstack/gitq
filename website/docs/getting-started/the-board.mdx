...
The server reads `config.json` from the [app root](/reference/configuration#the-app-root) -- `~/.mattstack/gitq`, from a checkout as much as from the compiled binary, unless `GITQ_APP_ROOT` moves it. Start from the example:
