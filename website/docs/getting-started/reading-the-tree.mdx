...
One block per stack, one line per branch, in stack order: `  <branch>: <situation>`. A branch that is currently checked out in one of your worktrees gets a suffix naming it, `  feat-api: behind-parent [in gitq-2]`, which is how you find out why a cascade is about to refuse to move that branch.
...
| `branch-deleted-remote` | The branch used to exist on the remote and no longer does. If the branch was recorded as merged, this is the normal end of its life and it is safe to remove from the stack; otherwise something deleted it out from under you. |
...
`preflight` predicts; it does not promise. It replays the rebase to look for conflicts, so it can miss cases that only appear once earlier branches have actually been rewritten.
